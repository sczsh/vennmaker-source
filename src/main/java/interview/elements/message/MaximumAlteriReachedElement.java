...
/**
 * Tests, if there are the maximum amount of alteri to continue the interview.
 * <p>
 ...
 */
public class MaximumAlteriReachedElement extends StandardElement implements
        IECategory_MetaElement {
    private static final long serialVersionUID = 1L;

    private JSpinner maximumAlteriSpinner;
    ...
    public MaximumAlteriReachedElement() {
    ...
    public boolean writeData() {
        ...
        if (InterviewController.getInstance().wasNextPressed()) {
            boolean result = this.shouldBeSkipped();

            if (result == true) {
                this.skipped = true;
                return true;
            }
            //	JOptionPane.showMessageDialog(null, Messages.getString("MaximumAlteriReachedElement.MaximumNotReached"));
            this.skipped = false;
            return false;
        }
        return true;
    ...
    public boolean shouldBeSkipped() {
        int value = Integer.valueOf(this.maximumAlteriSpinner.getValue().toString()).intValue();
    ...
    public boolean addToTree() {
        setElementNameInTree(Messages
                .getString("MaximumAlteriReachedElement.Name"));
    ...
    public String getInstructionText() {
        return Messages.getString("MaximumAlteriReachedElement.Description"); //$NON-NLS-1$
    }

    @Override
    public String toString() {
        return getElementNameInTree() + " "
                + Messages.getString("MaximumAlteriReachedElement.Name");
    }

    @Override
    public InterviewElementInformation getElementInfo() {
        int value = Integer.valueOf(maximumAlteriSpinner.getValue().toString()).intValue();
    ...
    public void setElementInfo(InterviewElementInformation information) {
        ...
        this.maximumAlteriSpinner.setValue(info.getValue());
    ...
    public JPanel getControllerDialog() {
        ...
        JLabel captionLabel = new JLabel(
                Messages.getString("MaximumAlteriReachedElement.MaximumNotReached")
                        + "("
    ...
    public JPanel getConfigurationDialog() {
        infoField = new JTextArea(
                Messages.getString("MaximumAlteriReachedElement.DummyText"));
        ...
        if (this.maximumAlteriSpinner == null)
            this.maximumAlteriSpinner = new JSpinner(sm);
        ...
        if (true) {
            ...
            JLabel infoLabel = new JLabel(
                    Messages.getString("MaximumAlteriReachedElement.CaptionLabel"));

            JLabel spinnerLabel = new JLabel(
                    Messages.getString("MaximumAlteriReachedElement.Spinner"));
            ...
            gbLayout.setConstraints(this.maximumAlteriSpinner, c);
            configPanel.add(this.maximumAlteriSpinner);
        }
        return configPanel;
    }
