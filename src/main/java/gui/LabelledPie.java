...
public class LabelledPie {
     ...
     *
     * @param x        Die x-position (Zentrum des imaginären Kreises, Sternpunkt)
     * @param y        Die y-position (Zentrum des imaginären Kreises, Sternpunkt)
     * @param r1       Der innere Radius (bis dorthin wird nichts gezeichnet)
     * @param r2       Der äußere Radius (danach wird nichts gezeichnet)
     * @param rot      Der Öffnungswinkel (in GRAD)
     * @param off      Der Rotationswinkel (in GRAD)
     * @param label    Das Label, darf nicht <code>null</code> sein.
     * @param fontsize Die Schriftgröße in Punkten
     */
    public LabelledPie(double x, double y, double r1, double r2, double rot,
                       double off, String label, int fontsize) {
        this.x = x;
        this.y = y;
        this.r1 = r1;
        this.r2 = r2;
    ...
    private void createShape() {
        ...
        // Arc2D inner = new Arc2D.Double(x - r1, y - r1, r1 * 2, r1 * 2, 0,
        // rot,Arc2D.PIE);
        Ellipse2D inner = new Ellipse2D.Double(x - r1, y - r1, r1 * 2, r1 * 2);
        ...
        // dummy ansatz.
        if ((outline.getBounds2D().getWidth() > (r2 - r1 - 10))
                && (this.label.length() > 4)) {
            this.label = this.label.substring(0, this.label.length() - 5) + "…"; //$NON-NLS-1$
            createShape();
            return;
        }
